...
export function bindWalkInput({canvas, joystick, knob, enabled, look, change=()=>{}}) {
  ...
  function stopMove(e){if(e.pointerId!==moveId)return;const id=moveId;moveId=null;movement.forward=movement.side=0;change();knob.style.transform='translate(0px, 0px)';release(id);}
  ...
  function updateStick(e){const rect=joystick.getBoundingClientRect(),radius=rect.width*.32;
    ...
    movement.side=distance?dx/distance*strength:0;movement.forward=distance?-dy/distance*strength:0;change();
