...
function pumpBaseDetail(){while(baseDetailActive<3&&baseDetailQueue.length){const r=baseDetailQueue.shift(),id=r.meshId,spec=baseDetailSpecs[id];baseDetailActive++;unpack('./data/startup/'+spec.file).then(buffer=>{const cache=new Map(),levels=spec.levels.map(l=>{const key=l.offset+':'+l.indexOffset;if(!cache.has(key))cache.set(key,makeGeometry(l,buffer));return cache.get(key);});if(!nearReady.has(id))r.geometries[0]=levels[0];r.geometries[1]=levels[1];if(spec.far)r.geometries[2]=makeGeometry(spec.far,buffer);baseDetailReady.add(id);invalidate(true);}).catch(error=>{baseDetailRetry.set(id,performance.now()+30000);console.warn('Finer village detail unavailable',error.message);}).finally(()=>{baseDetailPending.delete(id);baseDetailActive--;pumpBaseDetail();});}}
